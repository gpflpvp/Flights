...
export class RegisterPassengerComponent implements OnInit {
  ...
  form = this.fb.group({
    email: ['', Validators.compose([Validators.required,Validators.email,Validators.min(3),Validators.max(100)])],
  ...
  checkPassenger(): void {
    ...
    this.passengerService.findPassenger(params as { email: string }).subscribe(this.login, e => {
